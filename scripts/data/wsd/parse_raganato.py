...
def raganato_to_relik(input_file_data: str, input_file_gold_key: str, output_file: str):
    data_gen = read_from_raganato(
        input_file_data,
        key_path=input_file_gold_key,
    )
    data = [d[2] for d in data_gen]
    ...
    with open(output_file, "w") as f:
        for i, raganto_sentence in enumerate(data):
            relik_sentence = {"doc_id": i}
            doc_words = [
                raganto_token.annotated_token.text for raganto_token in raganto_sentence
            ]
            dco_pos = [
                pos_map[raganto_token.annotated_token.pos]
                for raganto_token in raganto_sentence
            ]
            doc_pos_wn = [
                raganto_token.annotated_token.pos for raganto_token in raganto_sentence
            ]
            doc_lemma = [
                raganto_token.annotated_token.lemma
                for raganto_token in raganto_sentence
            ]
            doc_span_annotations = []
            for token_id, raganto_token in enumerate(raganto_sentence):
                if raganto_token.labels is not None:
                    for label in raganto_token.labels:
                        doc_span_annotations.append([token_id, token_id + 1, label])

            # relik_sentence["doc_text"] = " ".join(doc_words)
            relik_sentence["doc_words"] = doc_words
            relik_sentence["doc_pos"] = " ".join(dco_pos)
            relik_sentence["doc_pos_wn"] = " ".join(doc_pos_wn)
            relik_sentence["doc_lemma"] = " ".join(doc_lemma)
...
def main():
    ...
    arg_parser.add_argument("input_file_data", type=str)
    arg_parser.add_argument("input_file_gold_key", type=str)
    arg_parser.add_argument("output_file", type=str)
    args = arg_parser.parse_args()

    raganato_to_relik(**vars(args))
